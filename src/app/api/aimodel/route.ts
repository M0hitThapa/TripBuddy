  ...
  try {
    ...
    for (const model of modelFallbacks) {
      try {
        ...
        // Enhanced validation
        const hasResp = typeof parsed.resp === 'string' && parsed.resp.length > 0
        const isFinal = parsed.ui === 'Final'
        const hasItinerary = Array.isArray(parsed.itinerary) && parsed.itinerary.length > 0
        ...
        if (isFinal) {
          if (hasResp && hasItinerary && hasBudget) {
            ...
            const hasRequiredFields = firstDay && 
              firstDay.title && 
              firstDay.morning && 
              firstDay.afternoon && 
              firstDay.evening
